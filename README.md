...
```
A function can only accept and return integers. a function input can also not be the direct input from a function from which ts called. you have to transfer the value into a new variable. a function can be no longer than 99 instructions. you can have a max of 2 functions per file minus the "main" . a function can not have the same name as the file it is in.
